...
public interface TaskGiver {
	default Task dispatchTask(TaskCenter tc, double[] paras) {
		Task task = new Task(paras);
		tc.addTask(task);
		return task;
	}

	default Task dispatchTask(TaskCenter tc, float[] paras) {
		double[] paras2 = new double[paras.length];
		for (int i = 0; i < paras2.length; i++) {
			paras2[i] = (double) paras[i];
		}
		Task task = new Task(paras2);
		tc.addTask(task);
		return task;
	}
