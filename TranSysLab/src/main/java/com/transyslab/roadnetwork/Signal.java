 ...
 *
 */
public class Signal extends CodedObject {
	...
	public Signal() {
		 ...
		 *
		 * public int clearState(int flag){ return (state_ &= ~flag); } public
		 * int setState(int flag){ return (state_ |= flag); } public int state()
		 * { return state_; } public void state(int s) { state_ = s; }
		 *
		 * // These two functions apply to traffic signal only
		 *
		 * public int stateForExit(int i){ return ((state_ >> (i * SIGNAL_BITS))
		 * & SIGNAL_STATE); } public int stateForExit(int s, int i){ return ((s
		 * >> (i * SIGNAL_BITS)) & SIGNAL_STATE); }
		 *
		 * // Iterator on the global array
		 *
		 * public RN_Signal prev(){ if (index_ > 0) return
		 * theNetwork.signal(index_ - 1); else return null; } public RN_Signal
		 * next(){ if (index_ < (theNetwork.nSignals() - 1)) return
		 * theNetwork.signal(index_ + 1); else return null; }
		 *
		 * public int index() { return index_; } public RN_CtrlStation station()
		 * { return station_;} public RN_Lane lane() { return lane_; } public
		 * RN_Segment segment(){ // Segment that contains this signal if
		 * (station_!=null) return station_->segment(); else if (lane_!=null)
		 * return lane_.segment(); else return null; } public RN_Link link(){
		 * return segment().getLink(); } // Relative distance from the end of
		 ...
		 * SIGN_PATH) { // A path related message. Use link index instead of id
		 ...
		 * SIGN_LANE_USE_PATH || s == SIGN_PATH) { // A path related message.
		 ...
		 * distance(){ // Distance from the end of the link return
		 * station_.distance(); } // Visibility distance public float
		 ...
		 *
		 * state_ = stateIoToInternal(s); if (state_ == SIGN_ERROR) return -1;
		 * // error
		 *
		 * if (l < 0) {
		 *
		 ...
		 *
		 * } else { if (!(lane_ = theNetwork.findLane(l)) || (lane_.getSegment()
		 * != segment())) { // cerr << "Error:: Unknown lane ID <" << l << ">. "
		 * ; return -1; } }
		 *
		 * addIntoNetwork();
		 *
		 * return 0; } public void addIntoNetwork(){ index_ =
		 * theNetwork.nSignals(); theNetwork.addSignal(this); if
		 * (isLinkWide()>0) station_.signal(0, this); else
		 * station_.signal(lane_.localIndex(), this); } public void print(){
		 *
		 * }
		 *
		 ...
		 * rightmost lane
		 *
		 * public int leftOrRight(){ if ((lane_.get_code()) ==
		 * (segment().leftLane().get_code())) { return -1; } else if
		 * ((lane_.get_code()) == (segment().rightLane().get_code())) { return
